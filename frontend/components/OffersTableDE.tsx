...
export default function OffersTableDE() {
  ...
  const [platform, setPlatform] = useState<"all" | "ebay">("all");
      ...
      <div className="overflow-x-auto rounded-xl border border-white/10">
        <table className="min-w-full text-sm">
          <thead className="bg-zinc-900/80 text-zinc-400 text-xs uppercase tracking-wide">
            <tr>
              <SortTh label="Titel" sortKey="title" current={sortKey} dir={sortDir} onSort={handleSort} />
              <Th>Artikel</Th>
              <SortTh label="Preis" sortKey="price" current={sortKey} dir={sortDir} onSort={handleSort} />
              <Th>Marktwert</Th>
              <SortTh label="Marge" sortKey="margin_pct" current={sortKey} dir={sortDir} onSort={handleSort} />
              <Th>Plattform</Th>
              <Th>Tags</Th>
              <SortTh label="Eingestellt" sortKey="posted_at" current={sortKey} dir={sortDir} onSort={handleSort} />
          ...
          <tbody>
            {loading && (
              <tr><td colSpan={10} className="p-8 text-center text-zinc-500">Laden…</td></tr>
            )}
            {!loading && filtered.map(r => (
              ...
              >
                ...
                <Td className="font-mono font-semibold whitespace-nowrap">{fmtEUR(r.price)}</Td>
                <Td className="font-mono text-zinc-400 whitespace-nowrap">{r.market_value ? fmtEUR(r.market_value) : "—"}</Td>
                <Td>
                ...
                <Td>
                  <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-blue-900/50 text-blue-300">
                    {r.platform.toUpperCase()}
                  </span>
                </Td>
                <Td>
                  <div className="flex gap-1 flex-wrap">
                    {r.is_new && <span className="px-2 py-0.5 rounded-full bg-blue-900/60 text-blue-200 text-xs font-medium">Neu</span>}
                    {r.is_urgent && <span className="px-2 py-0.5 rounded-full bg-red-900/60 text-red-200 text-xs font-medium">Dringend</span>}
                ...
                <Td className="text-xs text-zinc-500 whitespace-nowrap">
                  {r.posted_at ? new Date(r.posted_at).toLocaleString("de-DE") : "—"}
                </Td>
                ...
                <Td>
                  <div className="flex items-center gap-1" onClick={e => e.stopPropagation()}>
                    <ActionBtn
                      ...
                      tooltip={r.is_favorite ? "Aus Favoriten entfernen" : "Zu Favoriten hinzufügen"}
                    ...
                    {r.status === "hidden" ? (
                      <ActionBtn onClick={e => restoreOffer(r.id, e)} tooltip="Angebot wiederherstellen">
                        <Eye size={16} />
                      </ActionBtn>
                    ) : (
                      <ActionBtn onClick={e => hideOffer(r.id, e)} tooltip="Angebot ausblenden">
                        <EyeOff size={16} />
                      </ActionBtn>
                    )}
                  </div>
            ...
            {!loading && filtered.length === 0 && (
              <tr>
                <td colSpan={10} className="p-8 text-center text-zinc-500">
                  {showHidden ? "Keine versteckten Angebote." : showFavorites ? "Keine Favoriten." : "Keine Angebote."}
                </td>
              </tr>
            )}
          </tbody>
