      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8 border-b border-[#30363d] pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-white tracking-tight">Market Analyzer (Groq AI)</h1>
              <p className="text-xs text-gray-400">Szybkie badanie cen przed dodaniem na Watchlistę</p>
            </div>
          </div>
        ...
        <form onSubmit={handleAnalyze} className="bg-[#161b22] border border-[#30363d] rounded-xl p-5 mb-6 shadow-xl">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 items-end">
            <div className="sm:col-span-1">
              <label className="block text-[11px] font-semibold text-gray-400 uppercase tracking-wider mb-2">Przedmiot do wyceny</label>
              <input
                type="text"
                placeholder="np. PS5 Slim, Dyson Airwrap..."
                value={phrase}
            ...
            <div>
              <label className="block text-[11px] font-semibold text-gray-400 uppercase tracking-wider mb-2">Twój cel marży / ROI %</label>
              <input
                type="number"
                value={roi}
            ...
            <div>
              ...
              >
                {loading ? "Analizuję bazę..." : "Sprawdź cenę rynkową"}
              </button>
            </div>
          </div>
        </form>
        ...
        {error && (
          <div className="bg-red-500/10 border border-red-500/20 text-red-400 p-4 rounded-lg text-xs mb-6">
            {error}
          </div>
        )}

        {/* Raport z Wyceny od Groq */}
        {result && (
          <div className="bg-[#161b22] border border-[#30363d] rounded-xl p-6 shadow-xl animate-fade-in">
            <div className="mb-4">
              <span className="text-[10px] font-bold text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-2 py-0.5 rounded-full uppercase tracking-wider">Raport Wygenerowany pomyślnie</span>
              <h2 className="text-lg font-bold text-white mt-2">{result.main_product_name}</h2>
              <p className="text-[11px] text-gray-400">Wyliczono na podstawie {result.sample_size_evaluated} trafień z Twojego scrapera.</p>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
              <div className="bg-[#0d1117] border border-[#30363d] p-4 rounded-lg">
                <span className="text-[11px] font-medium text-gray-400 uppercase tracking-wider">Wartość rynkowa (Sprzedaż)</span>
                <div className="text-xl font-bold text-white mt-1">{result.estimated_market_value_pln} PLN</div>
              </div>
              <div className="bg-[#0d1117] border border-emerald-500/30 p-4 rounded-lg relative">
                <span className="text-[11px] font-medium text-emerald-400 uppercase tracking-wider">Kupuj maksymalnie za (Próg)</span>
            ...
            {result.detected_noise && result.detected_noise.length > 0 && (
              <div className="mb-4 bg-[#0d1117] p-3 rounded-lg border border-[#30363d]">
                <h4 className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider mb-1">Odrzucone akcesoria / szum cenowy:</h4>
            ...
            {result.tips && (
              <div className="border-t border-[#30363d] pt-3 text-[11px] text-gray-400">
                <span className="font-semibold text-gray-300">Rada od AI:</span> {result.tips}
              </div>
            )}
          </div>
        )}
